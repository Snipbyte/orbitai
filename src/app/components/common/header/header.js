...
export default function Header() {
    ...
    return (
        <>
            {/* Mobile Menu Overlay */}
            <div
                className={`fixed inset-0 z-30 transition-all duration-500 ${showMenu ? "opacity-100 visible" : "opacity-0 invisible"
                    }`}
                style={{ backgroundColor: "rgba(0,0,0,0.5)" }}
                onClick={closeAll}
            >
                <div
                    className={`w-4/5 sm:w-3/5 md:w-2/5 min-h-screen bg-themeColor transition-all duration-300 ${showMenu ? "translate-x-0" : "-translate-x-full"
                        }`}
            ...
            {/* Header */}
            <header
                className={`w-full sticky top-0 left-0 right-0 z-20 bg-themeColor text-white shadow-sm transform transition-transform duration-500 ${showHeader ? "translate-y-0" : "-translate-y-full"
                    }`}
            >
                {/* Top Banner */}
                <div className="w-full bg-desColor py-2">
                    <p className="text-sm text-white text-center px-2">
                        Discover the future with our cutting-edge AI website!
                    </p>
                </div>
